...
const ServiceDetailModal = ({ service, isOpen, onClose }: ServiceDetailModalProps) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="w-[90vw] max-w-md max-h-[85vh] max-h-[85dvh] overflow-y-auto overflow-x-hidden bg-background border-border p-0 shadow-2xl shadow-black/40 rounded-xl [&>button]:hidden">
        {/* Close Button */}
        <button
          onClick={onClose}
