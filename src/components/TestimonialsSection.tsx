        ...
        <div className="max-w-7xl mx-auto">
          ...
          loop: true
            ...
            <CarouselContent className="-ml-4 md:-ml-6">
              {testimonials.map((testimonial, index) => <CarouselItem key={index} className="pl-4 md:pl-6 basis-full md:basis-1/2 lg:basis-1/3">
          ...
          <div className="flex justify-center gap-2 mt-8">
            {Array.from({
            length: count
          }).map((_, index) => <button key={index} onClick={() => api?.scrollTo(index)} className={`w-2 h-2 rounded-full transition-all duration-300 ${index === current ? "bg-primary w-6" : "bg-accent-foreground/30 hover:bg-accent-foreground/50"}`} aria-label={`Go to testimonial ${index + 1}`} />)}
          </div>
        </div>
      </div>
    </section>;
...
const TestimonialCard = ({
  ...
  return <article className="relative h-full p-8 md:p-10 bg-accent-foreground/5 border border-accent-foreground/10 transition-all duration-500 hover:border-primary/30 group">
      {/* Glow effect on hover */}
      <div className="absolute inset-0 bg-gradient-to-br from-primary/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
      
      {/* Quote Icon */}
      <Quote size={32} className="text-primary/40 mb-6 transition-all duration-500 group-hover:text-primary/60 group-hover:scale-110" fill="currentColor" />
