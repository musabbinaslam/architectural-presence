...
const AboutSection = () => {
  ...
  const { ref: certsRef, isVisible: certsVisible } = useScrollAnimation({ threshold: 0.3 });
  ...
  return (
    <section id="about" className="py-32 md:py-40 section-padding relative">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          <span 
            className={`text-[10px] tracking-[0.3em] uppercase text-muted-foreground block transition-all duration-700 ${
          ...
          <div 
            className={`w-12 h-px bg-primary mt-4 mb-8 transition-all duration-700 delay-100 origin-left ${
          ...
          <div className="space-y-8">
            <h2 
              className={`font-display text-4xl md:text-5xl lg:text-6xl leading-[1.1] transition-all duration-700 delay-200 ${
            ...
            {/* Signature */}
            <div 
              className={`pt-6 border-t border-border inline-block transition-all duration-700 delay-[600ms] ${
        ...
        >
          <p 
            className={`text-[10px] tracking-[0.3em] uppercase text-muted-foreground text-center mb-10 transition-all duration-500 ${
              certsVisible ? "opacity-100" : "opacity-0"
            }`}
          >
            Certifications & Memberships
          </p>
          <div className="flex flex-wrap justify-center items-center gap-12 md:gap-16">
            {["Pakistan Engineering Council", "ISO 9001 Certified", "PCATP Member", "Safety First Certified"].map((name, index) => (
              <span 
                key={name} 
                className={`text-sm font-medium tracking-wider text-muted-foreground hover:text-foreground transition-all duration-500 cursor-default ${
                  certsVisible ? "opacity-50 translate-y-0" : "opacity-0 translate-y-4"
                }`}
